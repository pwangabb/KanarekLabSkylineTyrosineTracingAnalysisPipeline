...
#
#####
#R-Script to process Skyline dataset: Transpose, Normalize, and Filter by Rsq and CV
#Modified for Skyline consolidated input: Direct File Selection & Transpose
...
# ==============================================================================
# PART 1: File Selection and Transpose
# ==============================================================================

# 1. Select the file directly
print("Please select your consolidated Skyline .csv file...")
skyline_file <- file.choose()

# Set working directory to the file location for outputs
wd <- dirname(skyline_file)
setwd(wd)

outputpath <- paste(wd, "/RscriptOutput/", sep = "") 
if (file.exists(outputpath) == FALSE) {
  dir.create(outputpath)
}

# 2. Read the file
raw_input <- read.csv(skyline_file, check.names = FALSE, stringsAsFactors = FALSE)

print("Transposing Skyline data: Metabolite rows -> Metabolite columns...")

# Extract Metabolite names (Column 1) and Sample data
metabo_names <- raw_input[,1]
data_to_t <- raw_input[,-1]

# Perform Transpose: Rows become Samples, Columns become Metabolites
df <- as.data.frame(t(data_to_t))
colnames(df) <- metabo_names

# Clean data: Ensure numeric type and handle "N/F"
df[] <- lapply(df, function(x) {
  x[x == "N/F"] <- NA
  as.numeric(as.character(x))
})

# 3. View and Filter Rows
View(df)
...
if (input == "y")
{
  ...
  while(i == 0)
  {
    ...
    # Reset rowlogical for each attempt
    rowlogical <- rep(FALSE, nrow(df))
    
    for (k in 1:length(rowstoremove))
    {
      temp <- grepl(rowstoremove[k], rownames(df))
      rowlogical <- rowlogical | temp
    }
    
    print(rownames(df)[rowlogical])
